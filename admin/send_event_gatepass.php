...
// Fetch registration meta
$regStmt = $conn->prepare("SELECT id, team_name, module_selection, status FROM event_registrations WHERE id = ? LIMIT 1");
$regStmt->bind_param('i', $registrationId);
$regStmt->execute();
$regRes = $regStmt->get_result();
$registration = $regRes ? $regRes->fetch_assoc() : null;
$regStmt->close();
...
foreach ($attendees as $a) {
    $attendeeId = (int)$a['id'];
    $name = $a['full_name'] ?: 'Participant';
    $email = $a['email'];
    $teamName = $a['team_name'] ?? '';
    $module = $a['module_selection'] ?? '';
    $role = $a['label'] ?? ('Member ' . ($a['person_index'] ?? ''));
